...
app.get("/", function(req, res){
    //res.send(`<h2>Ta funcionando ${req.query['nome']} ${req.query['nome2']} ${req.query['sobrenome']}</h2>`);
    res.render('index');
});
...
app.post("/clientes/save", function(req, res){
    ...
    if(req.body.nome === "" || req.body.endereco === "" || req.body.sexo === "" || req.body.telefone === ""){
        //res.redirect("/clientes");
    ...
    if(clienteantigo != undefined){
        ...
        fakeData.map( o => o.id )    transforma em    const outralista = [{id: 1}, {id: 2}];
        ...fakeData.map( o => o.id )    transforma em    const listanumeros = [1,2];
        */
